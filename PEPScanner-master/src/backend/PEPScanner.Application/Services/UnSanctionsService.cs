...
namespace PEPScanner.API.Services
{
    ...
    public class UnSanctionsService : IUnSanctionsService
    {
            ...
            try
            {
                ...
                foreach (var unEntry in unEntries)
                {
                    ...
                    if (existingEntry == null)
                    {
                        ...
                        // Add alternate names
                        if (!string.IsNullOrEmpty(unEntry.Aliases))
                        {
                            newEntry.AlternateNames = unEntry.Aliases;
                        }
                    ...
                    else
                    {
                        ...
                        existingEntry.City = unEntry.City;
                        existingEntry.State = unEntry.State;
                        existingEntry.PostalCode = unEntry.PostalCode;
                        existingEntry.PositionOrRole = unEntry.Function;
                        existingEntry.SanctionReason = unEntry.Reason;
                        existingEntry.Comments = unEntry.Remarks;
                        existingEntry.DateLastUpdatedUtc = DateTime.UtcNow;
                        existingEntry.UpdatedBy = "System";

                        if (!string.IsNullOrEmpty(unEntry.Aliases))
                        {
                            existingEntry.AlternateNames = unEntry.Aliases;
                        }
        ...
        public async Task<List<UnSanctionsEntry>> SearchByNameAsync(string name)
        {
            try
            {
                ...
                return allEntries.Where(e => 
                    ...
                    e.Aliases?.Contains(name, StringComparison.OrdinalIgnoreCase) == true ||
                    e.Function?.Contains(name, StringComparison.OrdinalIgnoreCase) == true
                ).ToList();
            }
            catch (Exception ex)
            {
        ...
        private List<UnSanctionsEntry> ParseUnSanctionsJson(string jsonContent)
        {
            try
            {
                ...
                return unData?.Individuals ?? new List<UnSanctionsEntry>();
            }
            catch (Exception ex)
            {
    ...
    {
        public List<UnSanctionsEntry> Individuals { get; set; } = new();
        public List<UnSanctionsEntry> Entities { get; set; } = new();
        public List<UnSanctionsEntry> Vessels { get; set; } = new();
    }
}
